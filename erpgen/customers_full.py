...
def _agent_instructions(party: str, flow: str) -> str:
    return (
        ...
        f"({party} + Contact + Address in one file). A column NOT in the fixed "
        "mapping contract is dropped at import, so it is an error-severity "
        "unmapped_column. Fix every conflict; import once none of them is open or "
        "stale:\n"
        ...
        f"suggested doctype/label/fieldtype (the suggested create_command lists "
        f"them), then set_mapping('{flow}', column, '<doctype>.<fieldname>') "
        "using the suggested fieldname.\n"
        ...
        "needs a leaf (ERPNext rejects a Group node on a party). Create the leaf "
        "with create_record (parent set, is_group 0), then set_value that cell to "
        "the leaf's name. Do not create the group again.\n"
        "- duplicate_row -> the same party name appears on more than one row. "
        "Merge the rows (merge_rows with field_overrides to choose which cells "
        "survive), drop the extra row (skip_row), or retarget the review key with "
        ...
        "Never invent a value. Both mapping resolutions retire their conflict on "
        "the next map (the column is mapped). After resolving, re-run map and check "
        "each conflict's status: import once no error-severity conflict is open or "
        "stale. A corrected/waived conflict keeps its severity and stays in the "
        "analysis — that is expected, not a blocker."
    )
